/**
 * UmAlQura Calendar - JavaScript Implementation
...
function checkYearMonthRange(year, month) {
  checkYearRange(year);
  if (month < 1 || month > 12) {
...
function hijriToGregorian(hijriYear, hijriMonth, hijriDay) {
  checkYearMonthRange(hijriYear, hijriMonth);
...
function printUsage() {
  console.log(`
UmAlQura Calendar Converter
===========================
Convert between Gregorian and UmAlQura (Hijri) dates.

Usage:
  node umAlQuraCalendar.js <gregorian-date>
  node umAlQuraCalendar.js --hijri <year> <month> <day>
  
Examples:
  node umAlQuraCalendar.js 2024-01-15
  node umAlQuraCalendar.js "January 15, 2024"
  node umAlQuraCalendar.js --hijri 1445 7 3
  
Supported Gregorian range: April 30, 1900 - November 17, 2077
Supported Hijri range: 1318/01/01 - 1500/12/30
`);
}
...
function main() {
  ...
  try {
    if (args[0] === '--hijri') {
      if (args.length < 4) {
        console.error('Error: --hijri requires year, month, and day arguments');
        printUsage();
        process.exit(1);
      }
      ...
      if (isNaN(year) || isNaN(month) || isNaN(day)) {
        console.error('Error: Invalid date components');
        process.exit(1);
      }
      ...
      console.log(`Hijri Date:     ${year}/${month.toString().padStart(2, '0')}/${day.toString().padStart(2, '0')}`);
      console.log(`                (${day} ${HIJRI_MONTHS[month - 1]} ${year} AH)`);
      console.log(`Gregorian Date: ${gregorianDate.toISOString().split('T')[0]}`);
      ...
      if (isNaN(gregorianDate.getTime())) {
        console.error(`Error: Could not parse date "${dateStr}"`);
        process.exit(1);
      }
      ...
      console.log(`Gregorian Date: ${gregorianDate.toISOString().split('T')[0]}`);
      console.log(`                (${gregorianDate.toDateString()})`);
      console.log(`Hijri Date:     ${hijriDate.year}/${hijriDate.month.toString().padStart(2, '0')}/${hijriDate.day.toString().padStart(2, '0')}`);
      console.log(`                (${hijriDate.day} ${HIJRI_MONTHS[hijriDate.month - 1]} ${hijriDate.year} AH)`);
      console.log(`Days in month:  ${getDaysInMonth(hijriDate.year, hijriDate.month)}`);
      console.log(`Days in year:   ${getDaysInYear(hijriDate.year)}`);
      console.log(`Leap year:      ${isLeapYear(hijriDate.year) ? 'Yes' : 'No'}`);
    }
